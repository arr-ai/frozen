...
func nextHasherWith[T any](v T, h hasher, depth int, hf func(T) H128) hasher {
	if (depth+1)%levelsPerRound == 0 {
...
func (b *branch[T]) Combine(args *CombineArgs[T], n node[T], depth int) (_ node[T], matches int) {
	...
	case *branch[T]:
		...
		return ret, matches
	case *leaf1[T]:
		return b.with(args, n.data, depth, hasherFromCached(n.h0, depth))
	case *leaf2[T]:
		hashes := [2]H128{n.ha, n.hb()}
		ret := b
		for i, e := range n.data {
			var m int
			ret, m = ret.with(args, e, depth, hasherFromCached(hashes[i], depth))
			matches += m
		}
		return ret, matches
	case *leaf[T]:
...
func (b *branch[T]) vetH0(hf func(T) H128) {
	want := childrenH0(&b.p)
	if b.h0 != want {
...
func withFastBatched[T any](root *branch[T], v T, h hasher, hf func(T) H128) (node[T], int) {

...
TEXT ·aeshash32H128(SB),NOSPLIT,$0-24
	MOVQ	p+0(FP), AX
	PXOR	X0, X0
	PINSRD	$2, (AX), X0
	AESENC	·aeskeysched+0(SB), X0
...
TEXT ·aeshash64H128(SB),NOSPLIT,$0-24
	MOVQ	p+0(FP), AX
	PXOR	X0, X0
	PINSRQ	$1, (AX), X0
	AESENC	·aeskeysched+0(SB), X0
...
TEXT ·aeshashH128(SB),NOSPLIT,$0-32
	MOVQ	p+0(FP), AX
	MOVQ	s+8(FP), CX
	LEAQ	ret+16(FP), DX
	JMP	aeshashbodyH128<>(SB)

// func aeshashstrH128(p unsafe.Pointer) H128
TEXT ·aeshashstrH128(SB),NOSPLIT,$0-24
	MOVQ	p+0(FP), AX
	MOVQ	8(AX), CX	// length of string
	MOVQ	(AX), AX	// string data
	LEAQ	ret+8(FP), DX
...
// CX: length
// DX: address to put return value
TEXT aeshashbodyH128<>(SB),NOSPLIT,$0-0
	// Zero seed (no per-table hash seed for H128).
	PXOR	X0, X0
	PINSRW	$4, CX, X0			// 16 bits of length
	PSHUFHW $0, X0, X0			// repeat length 4 times total
	MOVO	X0, X1				// save unscrambled seed
